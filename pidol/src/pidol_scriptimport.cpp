...
//        cerr << id << endl;
        ...
        if (!isDirectImport && !scriptSheet.hasStringEntryForId(id)) {
          ...
          PidolTranslationSheetEntry str = scriptSheet.getStringEntryById(id);
...
int main(int argc, char* argv[]) {
  if (argc < 1) {
    cout << "Private Eye Dol script importer" << endl;
    cout << "Usage: " << argv[0]
      << endl;
    return 0;
  }
  
  scriptSheet = PidolTranslationSheet();
  scriptSheet.importCsv("script/script_main.csv");
  importScript("spec_main.txt");
  
  scriptSheet = PidolTranslationSheet();
  scriptSheet.importCsv("script/script_advscene.csv");
  importScript("spec_advscene.txt");
  
  scriptSheet = PidolTranslationSheet();
  scriptSheet.importCsv("script/script_visual.csv");
  importScript("spec_visual.txt");
  ...
  scriptSheet = PidolTranslationSheet();
  scriptSheet.importCsv("script/script_8x8.csv");
  importScript("spec_8x8.txt");
  ...
  scriptSheet = PidolTranslationSheet();
  scriptSheet.importCsv("script/script_creditstext.csv");
  importScript("spec_creditstext.txt");
  
  scriptSheet = PidolTranslationSheet();
  scriptSheet.importCsv("script/script_backutil.csv");
  importScript("spec_backutil.txt");
