...
int main(int argc, char* argv[]) {
  if (argc < 2) {
    cout << "Private Eye Dol title sprite prep tool" << endl;
    cout << "Usage: " << argv[0] << " <infile> [outfile]" << endl;
    cout << "If omitted, outfile is same as infile." << endl;
    
    cout << "Options: " << endl;
    cout << "  -forcepal " << "Forces all palettes to the specified index"
      << endl;
/*    cout << "  -xoffset       " << "X-offset applies to output" << endl;
  ...
  int forcedPal = -1;
  TOpt::readNumericOpt(argc, argv, "-forcepal", &forcedPal);
...
//    sprite.read(ifs);
    ...
    int palette = rawFlags & 0x7F;
    int priority = rawFlags & 0x80;
    int flagsHigh = (rawFlags & 0xFF00) >> 8;
    ...
    if (pattern >= 0x100) {
      std::cerr << "Error: out-of-range pattern" << endl;
      return 1;
    }
    
    if (forcedPal != -1) palette = forcedPal;
    
    ofs.writes8(y);
    ofs.writes8(x);
    ofs.writeu8(pattern);
    ofs.writeu8(palette | priority);
    ofs.writeu8(flagsHigh);
  }
