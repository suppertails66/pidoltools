...
int main(int argc, char* argv[]) {
  if (argc < 3) {
    cout << "Private Eye Dol script wrapper" << endl;
  ...
  string tableName = "table/pidol_en.tbl";
...
//  table.readUtf8(tableName.c_str());
  ...
  {
    // read size table
    PidolLineWrapper::CharSizeTable sizeTable;
    {
      TBufStream ifs;
    ...
    PidolLineWrapper::CharSizeTable sizeTableEmph;
    {
      TBufStream ifs;
    ...
    
    {
      ...
      PidolLineWrapper(ifs, results, table, sizeTable, sizeTableEmph,
                       isNonstd)();
