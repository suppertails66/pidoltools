...
PidolScriptReader::ResultString::ResultString()
  : originalOffset(0),
    originalSize(0),
    initialized(false),
    autoStarted(false),
...
PidolScriptReader::ResultRegion::ResultRegion()
  : regionType(regionType_none),
    soundCount(0),
    choiceCount(0) { }

PidolScriptReader::PidolScriptReader(
...
bool PidolScriptReader::operator()() {
  try {
    while (!src.eof()) {
      std::string line;
      src.getLine(line);
...
void PidolScriptReader::loadThingy(const BlackT::TThingyTable& thingy__) {
  thingy = thingy__;
}
  
void PidolScriptReader::outputNextSymbol(TStream& ifs) {
...
//  if (ifs.eof()) return;
  ...
  throw TGenericException(T_SRCANDLINE,
                          "PidolScriptReader::outputNextSymbol()",
                          "Line "
...
void PidolScriptReader::flushActiveScript() {
...
void PidolScriptReader::resetScriptBuffer() {
...
/*bool PidolScriptReader::checkSymbol(BlackT::TStream& ifs, std::string& symbol) {
...
void PidolScriptReader::terminateCurrentStringIfActive() {
  if (stringIsActive) {
    outputTerminateCommand();
    stringIsActive = false;
  }
...
void PidolScriptReader::outputTerminateCommand() {
  int opcode = thingy.matchTableEntry("[null]").id;
  currentScriptBuffer.writeu8(opcode);
}

void PidolScriptReader::outputOffCommand() {
  terminateCurrentStringIfActive();
...
void PidolScriptReader::outputWaitCommand(double secTime) {
  terminateCurrentStringIfActive();
...
void PidolScriptReader::outputSlotCommand(int slotNum) {
  terminateCurrentStringIfActive();
...
void PidolScriptReader::outputPalCommand(int palNum) {
  terminateCurrentStringIfActive();
...
void PidolScriptReader::processDirective(BlackT::TStream& ifs) {
...
//    processStartMsg(ifs);
    ...
    if (name.compare("area") == 0) {
      ...
      throw TGenericException(T_SRCANDLINE,
                              "PidolScriptReader::processDirective()",
                              "Line "
                                + TStringConversion::intToString(lineNum)
...
void PidolScriptReader::processLoadTable(BlackT::TStream& ifs) {
  std::string tableName = TParse::matchString(ifs);
  TThingyTable table(tableName);
  loadThingy(table);
}

void PidolScriptReader::processStartMsg(BlackT::TStream& ifs) {
/*  currentScriptSrcOffset = TParse::matchInt(ifs);
  TParse::matchChar(ifs, ',');
...
void PidolScriptReader::processEndMsg(BlackT::TStream& ifs) {
  flushActiveScript();
}

void PidolScriptReader::processIncBin(BlackT::TStream& ifs) {
...
void PidolScriptReader::processBreak(BlackT::TStream& ifs) {
  breakTriggered = true;
}

void PidolScriptReader::processStartRegion(BlackT::TStream& ifs) {
  flushActiveScript();
...
void PidolScriptReader::processEndRegion(BlackT::TStream& ifs) {
//  TParse::matchInt(ifs);
//  TParse::matchInt(ifs);
////  flushActiveScript();
//  breakTriggered = true;
...
/*void PidolScriptReader::processSetIndexList(BlackT::TStream& ifs) {
  currentScriptIndices.clear();
...
double PidolScriptReader::matchTime(BlackT::TStream& ifs) {
