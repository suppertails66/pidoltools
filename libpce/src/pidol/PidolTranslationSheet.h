#ifndef PIDOLTRANSLATIONSHEET_H
#define PIDOLTRANSLATIONSHEET_H
...
class PidolTranslationSheetEntry {
public:
  enum Type {
    type_none,
    type_string,
...
typedef std::vector<PidolTranslationSheetEntry>
  PidolTranslationSheetEntryCollection;

class PidolTranslationSheet {
protected:
  struct DictEntry {
...
public:
  ...
  const PidolTranslationSheetEntry& getStringEntryById(std::string id) const;
...
protected:
  ...
  PidolTranslationSheetEntryCollection entries;
  std::map<std::string, PidolTranslationSheetEntry> stringEntryMap;
