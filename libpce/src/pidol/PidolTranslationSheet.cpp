...
void PidolTranslationSheet::addStringEntry(std::string id, std::string content,
                    std::string prefix, std::string suffix,
                    std::string translationPlaceholder) {
  PidolTranslationSheetEntry entry;
  entry.type = PidolTranslationSheetEntry::type_string;
  entry.stringId = id;
...
void PidolTranslationSheet::addCommentEntry(std::string comment) {
  PidolTranslationSheetEntry entry;
  entry.type = PidolTranslationSheetEntry::type_comment;
  entry.stringContent
    = std::string("//==================================\n")
      + "// " + comment
      + std::string("\n//==================================");
...
void PidolTranslationSheet::addMarkerEntry(std::string content) {
  PidolTranslationSheetEntry entry;
  entry.type = PidolTranslationSheetEntry::type_marker;
  entry.stringId = content;
  entries.push_back(entry);
}

bool PidolTranslationSheet::hasStringEntryForId(std::string id) const {
  return (stringEntryMap.find(id) != stringEntryMap.end());
}

const PidolTranslationSheetEntry& PidolTranslationSheet
    ::getStringEntryById(std::string id) const {
  return stringEntryMap.at(id);
}

void PidolTranslationSheet::importCsv(std::string filename) {
  TCsv csv;
  {
    TBufStream ifs;
...
void PidolTranslationSheet::exportCsv(std::string filename,
                      PidolTranslationSheet::LinkData* linkData) const {
  ...
  for (PidolTranslationSheetEntryCollection::const_iterator it
        = entries.cbegin();
       it != entries.cend();
       ++it) {
    const PidolTranslationSheetEntry& entry = *it;
    switch (entry.type) {
    case PidolTranslationSheetEntry::type_string:
    {
      // col A = type
      ofs << "\"string\"";
...
//          << "\"";
      ...
      break;
    case PidolTranslationSheetEntry::type_comment:
    {
...
//      ofs << "\"" << entry.stringContent << "\"";
      ...
      break;
    case PidolTranslationSheetEntry::type_marker:
    {
      if (entry.stringId.compare("dict_section_start") == 0) {
        ...
        throw TGenericException(T_SRCANDLINE,
                                "PidolTranslationSheet::exportCsv()",
                                "unknown marker: "
                                  + entry.stringId);
      }
    }
...
int PidolTranslationSheet::numEntries() const {
  return entries.size();
}

/*void PidolTranslationSheet::addBoxDuplicates(
    std::map<std::string, std::string>& boxMap,
    std::string content,
    std::string cell) {
...
std::map<std::string, std::string>::iterator PidolTranslationSheet
  ::findBoxDuplicate(
    std::map<std::string, std::string>& boxMap,
    std::string content) {
  std::vector<std::string> boxes = splitByBoxes(content);
...
std::vector<std::string> PidolTranslationSheet
...
std::string PidolTranslationSheet::getDictionariedString(LinkData& linkData,
                                         std::string content) {
  std::string result;
...
std::vector<std::string> PidolTranslationSheet::splitByDictEntries(
...
/*std::string PidolTranslationSheet::getQuoteSubbedString(std::string content) {
...
PidolTranslationSheet::LinkData::LinkData()
  : linked(false) { }
