...
void Yuna2TranslationSheet::addStringEntry(std::string id, std::string content,
                    std::string prefix, std::string suffix,
                    std::string translationPlaceholder) {
  Yuna2TranslationSheetEntry entry;
  entry.type = Yuna2TranslationSheetEntry::type_string;
  entry.stringId = id;
...
void Yuna2TranslationSheet::addCommentEntry(std::string comment) {
  Yuna2TranslationSheetEntry entry;
  entry.type = Yuna2TranslationSheetEntry::type_comment;
  entry.stringContent
    = std::string("//==================================\n")
      + "// " + comment
      + std::string("\n//==================================");
...
void Yuna2TranslationSheet::addMarkerEntry(std::string content) {
  Yuna2TranslationSheetEntry entry;
  entry.type = Yuna2TranslationSheetEntry::type_marker;
  entry.stringId = content;
  entries.push_back(entry);
}

const Yuna2TranslationSheetEntry& Yuna2TranslationSheet
    ::getStringEntryById(std::string id) const {
  return stringEntryMap.at(id);
}

void Yuna2TranslationSheet::importCsv(std::string filename) {
  TCsv csv;
  {
    TBufStream ifs;
...
void Yuna2TranslationSheet::exportCsv(std::string filename,
                      Yuna2TranslationSheet::LinkData* linkData) const {
  ...
  for (Yuna2TranslationSheetEntryCollection::const_iterator it
        = entries.cbegin();
       it != entries.cend();
       ++it) {
    const Yuna2TranslationSheetEntry& entry = *it;
    switch (entry.type) {
    case Yuna2TranslationSheetEntry::type_string:
    {
      // col A = type
      ofs << "\"string\"";
...
//          << "\"";
      ...
      break;
    case Yuna2TranslationSheetEntry::type_comment:
    {
...
//      ofs << "\"" << entry.stringContent << "\"";
      ...
      break;
    case Yuna2TranslationSheetEntry::type_marker:
    {
      if (entry.stringId.compare("dict_section_start") == 0) {
        ...
        throw TGenericException(T_SRCANDLINE,
                                "Yuna2TranslationSheet::exportCsv()",
                                "unknown marker: "
                                  + entry.stringId);
      }
    }
...
int Yuna2TranslationSheet::numEntries() const {
  return entries.size();
}

/*void Yuna2TranslationSheet::addBoxDuplicates(
    std::map<std::string, std::string>& boxMap,
    std::string content,
    std::string cell) {
...
std::map<std::string, std::string>::iterator Yuna2TranslationSheet
  ::findBoxDuplicate(
    std::map<std::string, std::string>& boxMap,
    std::string content) {
  std::vector<std::string> boxes = splitByBoxes(content);
...
std::vector<std::string> Yuna2TranslationSheet
...
std::string Yuna2TranslationSheet::getDictionariedString(LinkData& linkData,
                                         std::string content) {
  std::string result;
...
std::vector<std::string> Yuna2TranslationSheet::splitByDictEntries(
...
/*std::string Yuna2TranslationSheet::getQuoteSubbedString(std::string content) {
...
Yuna2TranslationSheet::LinkData::LinkData()
  : linked(false) { }
