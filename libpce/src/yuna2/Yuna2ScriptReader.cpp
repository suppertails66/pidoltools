...
#define YUNA2READER_IGNORE_BAD_INPUT 1
...
Yuna2ScriptReader::ResultString::ResultString()
  : srcOffset(-1),
    srcPointer(-1),
    srcSize(-1),
    width(-1),
...
Yuna2ScriptReader::Yuna2ScriptReader(
...
bool Yuna2ScriptReader::operator()() {
  try {
    while (!src.eof()) {
      std::string line;
      src.getLine(line);
...
void Yuna2ScriptReader::loadThingy(const BlackT::TThingyTable& thingy__) {
  thingy = thingy__;
}
  
void Yuna2ScriptReader::outputNextSymbol(TStream& ifs) {
...
//    }
  ...
  #if YUNA2READER_IGNORE_BAD_INPUT
...
//  if (ifs.eof()) return;
  ...
  throw TGenericException(T_SRCANDLINE,
                          "Yuna2ScriptReader::outputNextSymbol()",
                          "Line "
...
void Yuna2ScriptReader::flushActiveScript() {
...
void Yuna2ScriptReader::resetScriptBuffer() {
...
/*bool Yuna2ScriptReader::checkSymbol(BlackT::TStream& ifs, std::string& symbol) {
...
void Yuna2ScriptReader::processDirective(BlackT::TStream& ifs) {
...
//  }
    ...
    throw TGenericException(T_SRCANDLINE,
                            "Yuna2ScriptReader::processDirective()",
                            "Line "
                              + TStringConversion::intToString(lineNum)
                              + ":\n  Unknown directive: "
                              + name);
...
void Yuna2ScriptReader::processLoadTable(BlackT::TStream& ifs) {
  std::string tableName = TParse::matchString(ifs);
  TThingyTable table(tableName);
  loadThingy(table);
}

void Yuna2ScriptReader::processStartMsg(BlackT::TStream& ifs) {
/*  currentScriptSrcOffset = TParse::matchInt(ifs);
  TParse::matchChar(ifs, ',');
...
void Yuna2ScriptReader::processEndMsg(BlackT::TStream& ifs) {
  flushActiveScript();
}

void Yuna2ScriptReader::processIncBin(BlackT::TStream& ifs) {
...
void Yuna2ScriptReader::processBreak(BlackT::TStream& ifs) {
  breakTriggered = true;
}

void Yuna2ScriptReader::processStartRegion(BlackT::TStream& ifs) {
  flushActiveScript();
...
void Yuna2ScriptReader::processEndRegion(BlackT::TStream& ifs) {
  TParse::matchInt(ifs);
//  TParse::matchInt(ifs);
////  flushActiveScript();
//  breakTriggered = true;
...
/*void Yuna2ScriptReader::processSetIndexList(BlackT::TStream& ifs) {
  currentScriptIndices.clear();
